...
def test_generation_maps_provider_refusal_to_none(monkeypatch):
    import src.task10_generation as generation

    chunks = [result("chunk-0", 0.8, "hybrid")]
    monkeypatch.setattr(generation, "retrieve", lambda query, top_k: chunks)
    monkeypatch.setattr(
        generation,
        "call_llm",
        lambda system, user: "Không có thông tin trong tài liệu nên không thể trả lời.",
    )

    output = generation.generate_with_citation("weather", top_k=1)
