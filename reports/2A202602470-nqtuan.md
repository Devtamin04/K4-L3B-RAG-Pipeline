...
| Generation/UI | Ollama Cloud, citation mapping, safe refusal; demo Streamlit theo mockup với truy vết pipeline, citation highlighting và tab so sánh 2 embedding | `src/task10_generation.py`, `app.py`, `demo_embeddings.py`, `demo_style.css` | Done |
...
- Kết quả trước/sau nếu có: 21/21 test pass; hybrid +0.035 điểm trung bình so với dense; threshold chọn 0.573.
- Lỗi đã phát hiện và cách xử lý: PDF scan không có text layer nên bổ sung HTML chính thức; citation sai thứ tự sau reorder nên gắn nhãn trước khi reorder; re-index được chuyển sang incremental; PhoBERT khai báo `max_seq_length=512` nhưng chỉ có 256 vị trí nên giới hạn lại để index không crash; LLM trả `[Document\u202fN]` nên nới regex để citation vẫn bấm được.

## Điều còn hạn chế

- Một hạn chế cụ thể của phần tôi làm: PageIndex cloud chưa được gọi thật vì không có `PAGEINDEX_API_KEY`; pipeline hiện kiểm thử degraded mode. Threshold `0.573` chỉ calibrate cho BGE-M3, trong khi `dangvantuan/vietnamese-embedding` cho cosine thấp hơn (0.681 so với 0.773 trên cùng câu hỏi).
